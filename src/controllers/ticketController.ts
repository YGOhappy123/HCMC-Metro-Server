...
import ticketService from '@/services/ticketService'; // Sử dụng ticketService
...
const issuedTicketController = {
  ...
  issueSubscriptionTicket: async (req: Request, res: Response, next: NextFunction) => {
    try {
      const data = req.body;
      const result = await ticketService.issueSubscriptionTicket(data);
      res.status(200).json({
        data: result.ticket,
